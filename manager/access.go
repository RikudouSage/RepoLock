...
func (receiver *access) CanUserAccessOrganization(ctx context.Context, userID, organizationID uuid.UUID, accessType AccessType) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed getting permission for user with id %s: %w", userID, err)
	}
	...
	default:
		return false, fmt.Errorf("invalid permission for user with id %s: %s", userID, permission)
	}
}
